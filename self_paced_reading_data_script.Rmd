...
```{r # Reading .csv dataset with results of the self-paced reading experiment}
spr <- read.csv("spr_data.csv")
head(spr)
```

```{r # Checking raw RTs for normality}
...
```{r # Log-transforming raw RTs to the base of 'e' - 'natural logarithm' to overcome skewness}
...
```{r # Contrast coding the variable 'verb_strength' (weak vs. strong)}

spr_log$verb_strength <- ifelse(spr_log$verb_strength == "weak", -0.5, 0.5)
spr_log$verb_strength
```

```{r # Operationalization of RTs of Spanish sentences with constructions in the self-paced reading experiment}
...
```{r # Calculating basic descriptive statistics}
...
```{r # Mixed-effects linear modeling}
